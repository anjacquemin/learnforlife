class ThemesController < ApplicationController
  ...
  def unlocked_theme_achievements_calculation(theme, theme_stats)
    locked_achievements = User.first.achievements.joins(:user_achievements).where(user_achievements: {unlocked: false}).where(achievement_category: theme.name)
    ...
    locked_achievements.each do |achievement|
      ...
      if achievement_type == "crown_percentage"
        ...
        if (theme_stats[:crowns] / (total_theme_crowns.nonzero? || 1)) > (achievement.count.to_i / 100)
          achievements_to_unlocked << unlock_achievement(achievement)
        end
      elsif achievement_type ==  "sphere_percentage"
        total_theme_spheres = theme.quizz_levels.joins(:quizz).where(quizz: {crown_or_sphere: "crown"}).count * 3
        if (theme_stats[:spheres] / (total_theme_spheres.nonzero? || 1)) > (achievement.count.to_i / 100)
          achievements_to_unlocked << unlock_achievement(achievement)
        end
...
def unlocked_global_achievements_calculation(themes_stats)
  ...
  locked_achievements = User.first.achievements.joins(:user_achievements).where(user_achievements: {unlocked: false}).where(achievement_category: "global")
